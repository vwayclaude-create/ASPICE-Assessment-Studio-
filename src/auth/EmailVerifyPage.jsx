...
export function EmailVerifyPage({ pending, onUpdatePending, onCancel }) {
  ...
  // Safety: if code is somehow missing (e.g. stale state from older build), regenerate it.
  useEffect(() => {
    if (!pending?.code || !/^\d{6}$/.test(pending.code)) {
      onUpdatePending({ ...pending, code: generateCode(), issuedAt: Date.now() });
    }
  }, [pending, onUpdatePending]);
    ...
    <div style={authPageStyle}>
      <div style={authCardStyle}>
        ...
        <div style={{ marginTop: 14, display: "flex", gap: 10 }}>
          <button
            type="button"
            onClick={() => { if (pending.code) setInput(pending.code); setError(""); }}
